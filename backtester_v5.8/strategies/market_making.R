...
initStore <- function(newRowList, series) {
  ...
  for (s in series) {
    ...
    periodVolatilities[[s]] <- NA  # or vector() for an empty vector
    liquidity[[s]] <- NA   # or vector() for an empty vector
    spreads[[s]] <- NA      # or vector() for an empty vector
...
calculateVolatility <- function(series, storeIter, volatilityLookback){
  ...
  # Is it a bad idea to have the entire volume series? Too high a granularity? -> No, because we should get the volatility at all periods?
  # Maybe add something to calculate the monthly volatility or something akin to this.
